...
class GECo:
    ...
    def predict(self,graph):
        ...
        with torch.no_grad():
            ...
            probs = softmax(out)
            # If node-level logits are returned, aggregate to class-level probs.
            if probs.dim() == 2:
    ...
    def visualize_results(self,graph,communities,comm_prob,explenation):
        ...
        Line2D([0], [0], marker='o', color='w',
           markerfacecolor=colors(i), markersize=10, label=f'Value: {val[1]:.4f}')
            for i, val in enumerate(sorted(comm_prob, key=lambda x: x[1], reverse=True)[:5])
        ]
        plt.legend(handles=custom_legend, loc="best",fontsize= 8)
        plt.tight_layout()
        plt.show()
    ...
    def explain(self,graph,pred,visualize,verbose=False):
        ...
        if verbose:
            comm_prob = sorted(comm_prob, key=lambda x: x[1], reverse=True)
            print(f"Tau {tau}")
